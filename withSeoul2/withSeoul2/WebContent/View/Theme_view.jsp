...
<head>
	<meta charset="utf-8">
	<title>테마로 찾기</title>
	<link rel="stylesheet" type="text/css" href="../Css/theme_view.css">
</head>
<body>
<%
Javabeans db = (Javabeans)session.getAttribute("db");
	...
	if(pagenum == null || theme==null || db==null){
		flag = false;
		%>
		<script>
			alert('page theme오류');
	...
	if(theme.equals("tour")){
		sql = "select * from post_tbl where tour = 1";
		title = "관광지/유적지";
	}
	else if(theme.equals("landmark")){
		sql = "select * from post_tbl where landmark = 1";
		title = "랜드마크";
	}
	else if(theme.equals("art")){
		sql = "select * from post_tbl where art = 1";
		title = "문화/예술";
	}
	else if(theme.equals("sports")){
		sql = "select * from post_tbl where sports = 1";
		title = "스포츠";
	}
	...
	if(num <= 0){
		...
		<script>
			...
			document.location.href="Theme_view.jsp?page=1&theme=<%= theme %>";
		</script>
		<%
		...
		<script>
			alert('페이지의 범위를 벗어났습니다.');
			document.location.href="Theme_view.jsp?page=<%= endnum/10+1 %>&theme=<%= theme %>";
		</script>
		<%
	}
	if(flag){
	sql = "select * from post_tbl where "+theme+" = 1 limit "+(num-1)*10+",10";
...
%>
	...
	<table>
		<tr>
			<td>1</td>
			<td><a href="Theme_view.jsp?page=1&theme=<%= theme %>">&lt;&lt;</a></td>
			<td><a href="Theme_view.jsp?page=<%= num-1 %>&theme=<%= theme %>">&lt;</a></td>
			<td style="width: 90px;"><%= num %> page</td>
			<td><a href="Theme_view.jsp?page=<%= num+1 %>&theme=<%= theme %>">&gt;</a></td>
			<td><a href="Theme_view.jsp?page=<%= endnum/10+1 %>&theme=<%= theme %>">&gt;&gt;</a></td>
			<td><%= endnum/10+1 %></td>
		</tr>
	</table>
	</div>
