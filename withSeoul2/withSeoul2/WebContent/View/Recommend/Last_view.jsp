...
<head>
	<meta charset="utf-8">
	<link rel="stylesheet" type="text/css" href="../../Css/last_view.css">
	<title>추천명소-최신순</title>
</head>
<body>
<%
Javabeans db = (Javabeans)session.getAttribute("db");
	...
	if(pagenum == null||db==null){
		...
		<script>
			alert('page오류');
			document.location.href="Last_view.jsp?page=1";
		</script>
		<%
	}
	int startnum=0;
	...
	if(num <= 0){
		...
		<script>
			alert('페이지의 범위를 벗어났습니다.');
			document.location.href="Last_view.jsp?page=<%= endnum/10+1 %>";
		</script>
		<%
	}
	if(flag){
	sql = "select * from post_tbl limit "+(num-1)*10+",10";
...
%>
	...
	<table>
		<tr>
			<td>1</td>
			<td><a href="Last_view.jsp?page=1">&lt;&lt;</a></td>
			<td><a href="Last_view.jsp?page=<%= num-1 %>">&lt;</a></td>
			<td style="width: 90px;"><%= num %> page</td>
			<td><a href="Last_view.jsp?page=<%= num+1 %>">&gt;</a></td>
			<td><a href="Last_view.jsp?page=<%= endnum/10+1 %>">&gt;&gt;</a></td>
			<td><%= endnum/10+1 %></td>
		</tr>
	</table>
	</div>
