...
<head>
	<meta charset="utf-8">
	<title>추천명소-연령대별</title>
	<link rel="stylesheet" type="text/css" href="../../Css/old_view.css">
</head>
<body>
<%
Javabeans db = (Javabeans)session.getAttribute("db");
	...
	String agenum = request.getParameter("age");
	String sql = "select * from post_tbl where age = "+agenum;
	if(pagenum == null||db==null){
		flag = false;
		%>
		<script>
			alert('page오류');
			document.location.href="Old_view.jsp?page=1&age=0";
		</script>
		<%
	}
	if(agenum == null){
	...
	int anum=Integer.parseInt(agenum);
	if(anum < 0 || anum > 6){
		flag = false;
		%>
		<script>
			alert('연령대의 범위를 벗어났습니다.');
			document.location.href="Old_view.jsp?page=1&age=0";
		</script>
		<%
	}else if(num <= 0){
		...
		<script>
			...
			document.location.href="Old_view.jsp?page=1&age=<%= anum %>";
		</script>
		<%
		...
		<script>
			alert('페이지의 범위를 벗어났습니다.');
			document.location.href="Old_view.jsp?page=<%= endnum/10+1 %>&age=<%= anum %>";
		</script>
		<%
	}
	if(flag){
	sql = "select * from post_tbl where age = "+anum+" limit "+(num-1)*10+",10";
...
%>
	...
	<div class="selectbar">
		<ul>
			<li>연령대별</li>
			<li><a href="Old_view.jsp?page=1&age=0">10대미만</a></li>
			<li><a href="Old_view.jsp?page=1&age=1">10대</a></li>
			<li><a href="Old_view.jsp?page=1&age=2">20대</a></li>
			<li><a href="Old_view.jsp?page=1&age=3">30대</a></li>
			<li><a href="Old_view.jsp?page=1&age=4">40대</a></li>
			<li><a href="Old_view.jsp?page=1&age=5">50대이상</a></li>
		</ul>
	</div>
	<div id="footer">
	<table>
		<tr>
			<td>1</td>
			<td><a href="Old_view.jsp?page=1&age=<%= anum %>">&lt;&lt;</a></td>
			<td><a href="Old_view.jsp?page=<%= num-1 %>&age=<%= anum %>">&lt;</a></td>
			<td style="width:90px;"><%= num %> page</td>
			<td><a href="Old_view.jsp?page=<%= num+1 %>&age=<%= anum %>">&gt;</a></td>
			<td><a href="Old_view.jsp?page=<%= endnum/10+1 %>&age=<%= anum %>">&gt;&gt;</a></td>
			<td><%= endnum/10+1 %></td>
		</tr>
	</table>
	</div>
